...
class OneLayer(nn.Module):
    def __init__(self, n_classes=5):
        super().__init__()
        self.f = nn.Sequential(nn.Conv3d(1, n_classes, kernel_size=1, stride=1))
...
class Residual(torch.nn.Module):
    def __init__(self, in_channels):
        ...
        self.final = nn.Sequential(nn.BatchNorm3d(n), nn.GELU())
...
class ResLayer(nn.Module):
    ...
    def make_layer(self, in_channels, out_channels, n_blocks, stride):
        layers = []
        layers.append(
            ResBlock(in_channels, out_channels, stride=stride, downsample=True)
        )
