...
def accuracy(prediction, target, n_classes=5):
    ...
    target = (
        nn.functional.one_hot(target, num_classes=n_classes)
        .float()
        .permute(0, 4, 1, 2, 3)
    )
    preds = (
        nn.functional.one_hot(prediction, num_classes=n_classes)
        .float()
        .permute(0, 4, 1, 2, 3)
    )
...
def iou(logits, target, smooth=1):
    ...
    if len(logits.shape) == 5:
        target = (
            nn.functional.one_hot(target.squeeze(), num_classes=logits.shape[1])
            .float()
            .permute(0, 4, 1, 2, 3)
        )
        preds = (
            nn.functional.one_hot(preds.squeeze(), num_classes=logits.shape[1])
            .float()
            .permute(0, 4, 1, 2, 3)
        )
    else:
