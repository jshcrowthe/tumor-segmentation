...
class FocalTverskyLoss(nn.Module):
    def __init__(self, reduce=True, alpha=0.5, gamma=2, smooth=1, weight=None):
        super().__init__()
        self.weight = weight
        self.alpha = alpha
        self.gamma = gamma
    ...
    def forward(self, prediction, target):
        ...
        if len(prediction.shape) == 5:
            target = (
                nn.functional.one_hot(target.squeeze(), num_classes=prediction.shape[1])
                .float()
                .permute(0, 4, 1, 2, 3)
            )
        else:
        ...
        if self.weight is not None:
            tversky = ((1 - tversky).pow(self.gamma) * self.weight).sum(
                dim=1
            ) / self.weight.sum()
        else:
...
class DiceLoss(nn.Module):
    def __init__(self, reduce=True, gamma=1, weight=None):
        ...
        self.loss = FocalTverskyLoss(
            alpha=0.5, gamma=gamma, reduce=reduce, weight=weight
        )
